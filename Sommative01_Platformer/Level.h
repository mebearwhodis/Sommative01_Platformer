...
public:
    ...
    //Constructor
    Level(const sf::Texture& background_texture);
	...
	static void SetBackgroundOrigin(const sf::Vector2f origin) { background_sprite_.setOrigin(origin); }
	static void SetBackgroundPosition(const sf::Vector2f position) { background_sprite_.setPosition(position); }
	...
	static void LoadLevelFromJson(const std::string& file_name, Texture texture);
};
